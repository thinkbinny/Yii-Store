...
class ActiveField extends Component
{
     ...
     */
    public $template = "{label}\n<div {width} class=\"layui-input-inline\">{input}\n{error}</div>\n{hint}";//
    /**
    ...
    public $textareaOptions = ['class' => 'layui-textarea'];
    /**
     ...
     * @see \yii\helpers\Html::renderTagAttributes() for details on how attributes are being rendered.
     */
    public $errorOptions = ['class' => 'layui-help-block'];
    /**
     ...
     * @see \yii\helpers\Html::renderTagAttributes() for details on how attributes are being rendered.
     */
    public $hintOptions = ['class' => 'layui-form-mid layui-word-aux'];
    /**
    ...
    public function render($content = null)
    {
        if ($content === null) {
            ...
            if (!isset($this->parts['{width}'])) {
                $this->width(null);
            }
            $this->parts['{width}'] = "style=\"width:{$this->parts['{width}']};\"";
    ...
    public function placeholder($options){
        if(!isset($options['placeholder'])){
            if($this->label){
                $options = array_merge($options,['placeholder'=>'请输入'.$this->label]);
            }else{
                $label   = $this->model->getAttributeLabel(Html::getAttributeName($this->attribute));
                $options = array_merge($options,['placeholder'=>'请输入'.$label]);
            }
        }
        return $options;
    }
    ...
    public function width($content, $options = [])
    {
        if ($content === false) {
            $this->parts['{width}'] = 'auto';
            return $this;
        }else{
            if(empty($content)){
                $this->parts['{width}'] = 'auto';
                ...
                if(is_int($content)){
                    $this->parts['{width}'] = $content.'px';
                }else{
                    $this->parts['{width}'] = $content;
                }
            }
            return $this;
        }
    ...
    {
        $options = $this->placeholder($options);
        $options = array_merge($this->inputOptions, $options);
        $this->addAriaAttributes($options);
        $this->adjustLabelFor($options);
