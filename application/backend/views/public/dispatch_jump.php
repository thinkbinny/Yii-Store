...
<head>
    ...
    <title><?php echo($info['msgTitle']); ?></title>
    <style type="text/css">
        *{ padding: 0; margin: 0; }
...
<div class="system-message">
    ...
    <?php if(isset($info['status']) && $info['status']==1) {?>
        <h1>恭喜您!</h1>
        <p class="success"><?php echo($info['message']); ?></p>
    <?php }else{?>
        <h1>抱歉,出错啦!</h1>
        <p class="error"><?php echo($info['message']); ?></p>
    <?php }?>

    <p class="jump">
        <b id="wait"><?php echo($info['waitSecond']); ?></b> 秒后页面将自动跳转
    </p>
    <div>
        <a id="href" href="<?php echo($info['jumpUrl']); ?>">立即跳转</a>
