...
$delivery_id   = Html::getInputId($model,'delivery_id');
$commission    = Html::getInputId($model,'commission');
$js = <<<JS
function mySubmit(){ 
   $('.ajax-submit').click();
}
...
layui.use('form', function() {
    ...
    //是否包邮
    form.on('radio(formIsFreeShipping)', function (data) {
        var _this = $('.field-{$delivery_id}');
    ...
    //分销
    form.on('radio(formIsFenxiao)', function (data) {
        var _this = $('.field-{$commission}');
...
$(".layui-submit-goods-ajax").submit(function () {
        ...
        if(skuType === '2'){
            ...
            //SKU数组
            var sku_list = skuData.sku_list;//console.log(sku_list);
            for(var i in sku_list){ 
                ...
                Data.push({
                    ...
                    'name':'{$fieldSkuMany}[data]['+i+'][sku_id]',
                    'value':sku_list[i].sku_id
                });
                //SKU参数
                var form = sku_list[i].form;                
                    ...
                    'name':'{$fieldSkuMany}[data]['+i+'][form][orig_price]',
                    ...
                    'name':'{$fieldSkuMany}[data]['+i+'][form][stock]',
                    'value':form.stock
                    }); 
...
<div class="goods-form" style="">
    ...
    if(isset($model->id)){
    $model->image_data = GoodsImage::getImageId($model->id);
    }
    echo $form->field($model, 'image_data')
    ...
    echo $form->field($model, 'sku_type',['options'=>['style'=>'margin-bottom:15px;']])->radioList($model->getSkuType(),[
        'item' => function($index, $label, $name, $checked, $value)
        {
            $checked=$checked?"checked":"";
    ...
    <!--单规则-->
        <div class="goods-sku goods-sku-single <?=$model->sku_type==1?'active':'';?>">
            ...
            echo  $form->field($model, 'orig_price')
                ->textInput([
                    'maxlength' => true,
                    'placeholder'=>'请输入商品原价',
                    'onkeyup'=>"this.value= this.value.match(/\d+(\.\d{0,2})?/) ? this.value.match(/\d+(\.\d{0,2})?/)[0] : ''"
            ...
            echo  $form->field($model, 'stock')
                ...
                //->label('库存数量')
                ->width(500);
            echo  $form->field($model, 'goods_weight')
                ->textInput([
                    'maxlength' => true,
                ...
                ->label('商品重量(Kg) ')
                ->width(500);
            echo  $form->field($model, 'productcode')
        ...
        <div id="many-sku" class="goods-sku goods-sku-many <?=$model->sku_type==2?'active':'';?>">
            <div class="goods-sku-box clearfix">
                ...
                <div v-if="!isSpecLocked" class="sku-group-button clearfix">
                    <button type="button" data-url="<?=Url::toRoute(['goods-attr/select','model_type'=>1])?>" class="sku-btn add-sku-group">添加规格</button>
                </div>
                ...
                <div v-if="sku_list.length > 0" class="sku-scrollable clearfix">
                    ...
                    <div class="sku-batch form-inline">
                        ...
                        <div class="sku-form-group">
                            <input class="layui-input sku-input" type="text" v-model="batchData.orig_price" placeholder="商品原价">
                        </div>
                        <div class="sku-form-group">
                            <input class="layui-input sku-input" type="text" v-model="batchData.stock" placeholder="库存数量">
                        </div>
                        <div class="sku-form-group">
                    ...
                    <table class="sku-tabel">
                        ...
                        <tr v-for="(item, index) in sku_list">
                            ...
                            <td>
                                <input onkeyup="this.value= this.value.match(/\d+(\.\d{0,2})?/) ? this.value.match(/\d+(\.\d{0,2})?/)[0] : ''" type="text" class="ipt-w80 layui-input" v-model="item.form.orig_price">
                            </td>
                            <td>
                                <input onkeyup="value=this.value.replace(/\D/g,'')" type="text" class="ipt-w80 layui-input" v-model="item.form.stock" required>
                            </td>
                            <td>
    ...
    if($model->is_free_shipping == null){
        $model->is_free_shipping = 0;
    }
    echo $form->field($model,'is_free_shipping')
        ->radioList($model->getIsFreeShipping(),[
            'item' => function($index, $label, $name, $checked, $value)
            {
                $checked=$checked?"checked":"";
                $return = "<input name=\"{$name}\" value=\"{$value}\" title=\"{$label}\" {$checked} type=\"radio\" lay-filter=\"formIsFreeShipping\">";
                return $return;
            }
        ]);
    ...
    if($model->is_free_shipping==1){
        $this->registerCss("
            .field-{$delivery_id}{display: none;}
        ");
    }
    ...
    echo $form->field($model,'is_fenxiao')
        ->radioList($model->getIsFenxiao(),[
            ...
            {
                $checked=$checked?"checked":"";
                $return = "<input name=\"{$name}\" value=\"{$value}\" title=\"{$label}\" {$checked} type=\"radio\" lay-filter=\"formIsFenxiao\">";
                return $return;
            }
        ])->hint('注：如需使用分销功能必须在 [分销中心 - 分销设置] 中开启<br>注：如不开启单独分销则默认使用全局分销比例')
        ->width(850);
    $commission    = Html::getInputId($model,'commission');
    if($model->is_fenxiao==0){
        $this->registerCss("
            .field-{$commission}{display: none;}
        ");
    }
    $commission =   $form->field($model, 'commission')
        ->textInput([
            'maxlength' => true,
            'placeholder'=>'请输入商品价格',
            'onkeyup'=>"this.value= this.value.match(/\d+(\.\d{0,2})?/) ? this.value.match(/\d+(\.\d{0,2})?/)[0] : ''"
        ...
        ->hint('')
        ->width(200);
    echo Html::tag('div',$commission,['style'=>'height:45px;']);
