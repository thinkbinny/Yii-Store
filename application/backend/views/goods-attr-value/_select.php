...
$InputName       =  Html::getInputName($model,'value');
$InputNameAttrId =  Html::getInputName($model,'attr_id');
$js = <<<JS
...
function mySuccess(option,_this) {
    ...
    if(option.params !== null){        
        ...
        var value = null;
        for(index in attr){
            if(attr[index].attr_id == {$attr_id}){
             value = attr[index].attr_items;
            }            
        }

        if(value!==null){
            for (index in value){
                $('#attr'+value[index].attr_value_id).find('input').attr('disabled',true);
            }
            layui.use('form', function() {
                var form = layui.form;
                form.render(); 
...
<div class="layuimini-container" style="border:none;">
    <div class="select-body">
        <div class="checkbtn">
            ...
            <button style="margin-left: 10px;" type="button" data-attribute="attr_id" data-attribute-value="<?=$attr_id?>" data-url="<?=Url::to(['create'])?>" class="add-attr">添加属性值</button>
        </div>
