...
use extensions\excel\PHPExcel\ReferenceHelper;
use extensions\excel\PHPExcel\Settings;
use extensions\excel\PHPExcel\Cell;
use extensions\excel\PHPExcel\Exception;
use extensions\excel\PHPExcel\Shared\File;
use extensions\excel\PHPExcel\Style\NumberFormat;
use extensions\excel\PHPExcel\Style;
use extensions\excel\PHPExcel\Style\Color;
use extensions\excel\PHPExcel\Style\Borders;
use extensions\excel\PHPExcel\Style\Protection;
 ...
 */
class Excel2007 extends Reader_Abstract implements IReader
{
    /**
            ...
            if ($rel["Type"] == "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument") {
                ...
                if ($xmlWorkbook->sheets) {
                    ...
                    foreach ($xmlWorkbook->sheets->sheet as $eleSheet) {
                        ...
                        $xml = new XMLReader();
                        $res = $xml->xml($this->securityScanFile('zip://'.File::realpath($pFilename).'#'."$dir/$fileWorksheet"), null, Settings::getLibXmlLoaderOptions());
            ...
            switch ($rel["Type"]) {
                case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/theme":
                    ...
                    if (is_object($xmlTheme)) {
                        ...
                        self::$theme = new Excel2007_Theme($themeName, $colourSchemeName, $themeColours);
                    }
                    break;
            }
        }
    ...
    //                                    echo 'Value is ', $value, PHP_EOL;
                            ...
                            if ($xmlSheet && $xmlSheet->colBreaks && $xmlSheet->colBreaks->brk && !$this->readDataOnly) {
                                foreach ($xmlSheet->colBreaks->brk as $brk) {
                                    if ($brk["man"]) {
                                        $docSheet->setBreak(Cell::stringFromColumnIndex((string) $brk["id"]) . "1", PHPExcel_Worksheet::BREAK_COLUMN);
                                    }
                                }
                            }
                ...
                switch ($contentType["ContentType"]) {
                    case "application/vnd.openxmlformats-officedocument.drawingml.chart+xml":
                        if ($this->includeCharts) {
                            ...
                            $objChart = Excel2007_Chart::readChart($chartElements, basename($chartEntryRef, '.xml'));
