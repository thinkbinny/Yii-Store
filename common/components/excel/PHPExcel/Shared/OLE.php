...
class OLE
{
    ...
    */
    public function read($file)
    {
        $fh = fopen($file, "r");
        if (!$fh) {
            throw new \extensions\excel\PHPExcel\Reader\Exception("Can't open file $file");
        }
        $this->_file_handle = $fh;

        $signature = fread($fh, 8);
        if ("\xD0\xCF\x11\xE0\xA1\xB1\x1A\xE1" != $signature) {
            throw new \extensions\excel\PHPExcel\Reader\Exception("File doesn't seem to be an OLE container.");
        }
        fseek($fh, 28);
        if (fread($fh, 2) != "\xFE\xFF") {
            // This shouldn't be a problem in practice
            throw new \extensions\excel\PHPExcel\Reader\Exception("Only Little-Endian encoding is supported.");
        }
    ...
    * use \extensions\excel\PHPExcel\Shared\OLE\PPS_File::getStream().
    ...
    */
    public function getStream($blockIdOrPps)
    {
        ...
        if (!$isRegistered) {
            stream_wrapper_register('ole-chainedblockstream', '\extensions\excel\PHPExcel\Shared\OLE\ChainedBlockStream');
            $isRegistered = true;
        }
        ...
        if ($blockIdOrPps instanceof \extensions\excel\PHPExcel\Shared\OLE\PPS) {
        ...
        for ($pos = 0;; $pos += 128) {
            ...
            switch ($type) {
                case self::OLE_PPS_TYPE_ROOT:
                    $pps = new \extensions\excel\PHPExcel\Shared\OLE\PPS_Root(null, null, array());
                    $this->root = $pps;
                    break;
                case self::OLE_PPS_TYPE_DIR:
                    $pps = new \extensions\excel\PHPExcel\Shared\OLE\PPS(null, null, null, null, null, null, null, null, null, array());
                    break;
                case self::OLE_PPS_TYPE_FILE:
                    $pps = new \extensions\excel\PHPExcel\Shared\OLE\PPS_File($name);
                    break;
                default:
                    continue;
            }
