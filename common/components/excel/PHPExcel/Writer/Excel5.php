<?php
namespace extensions\excel\PHPExcel\Writer;
use extensions\excel\PHPExcel;
use extensions\excel\PHPExcel\Cell;
use extensions\excel\PHPExcel\RichText;
use extensions\excel\PHPExcel\RichText\Run;
use extensions\excel\PHPExcel\Writer\Excel5\Parser;
use extensions\excel\PHPExcel\Writer\Excel5\Workbook;
use extensions\excel\PHPExcel\Writer\Excel5\Worksheet;
use extensions\excel\PHPExcel\Calculation;
use extensions\excel\PHPExcel\Calculation\Functions;
use extensions\excel\PHPExcel\Shared\OLE\PPS\File;
use extensions\excel\PHPExcel\Shared\OLE;
use extensions\excel\PHPExcel\Shared\Escher\DgContainer\SpgrContainer\SpContainer;
use extensions\excel\PHPExcel\Shared\Escher\DgContainer\SpgrContainer;
use extensions\excel\PHPExcel\Shared\Escher\DggContainer\BstoreContainer;
use extensions\excel\PHPExcel\Shared\Escher\DggContainer\BstoreContainer\BSE;
use extensions\excel\PHPExcel\Shared\Escher\DggContainer\BstoreContainer\BSE\Blip as BSE_Blip;
use extensions\excel\PHPExcel\Shared\Escher\DgContainer;
use extensions\excel\PHPExcel\Shared\Escher\DggContainer;
use extensions\excel\PHPExcel\Shared\Escher;
use extensions\excel\PHPExcel\Shared\Drawing as Shared_Drawing;
use extensions\excel\PHPExcel\Shared\Excel5 as Shared_Excel5;
use extensions\excel\PHPExcel\Worksheet\BaseDrawing;
use extensions\excel\PHPExcel\Worksheet\Drawing;
use extensions\excel\PHPExcel\Worksheet\MemoryDrawing;
//use extensions\excel\PHPExcel\Worksheet\MemoryDrawing;
 ...
 */
class Excel5 extends Writer_Abstract implements IWriter
{
    /**
     * PHPExcel object
     *
    ...
    public function save($pFilename = null)
    {
        ...
        $this->writerWorkbook = new Workbook($this->phpExcel, $this->strTotal, $this->strUnique, $this->strTable, $this->colors, $this->parser);

        // Initialise worksheet writers
        $countSheets = $this->phpExcel->getSheetCount();
        for ($i = 0; $i < $countSheets; ++$i) {
            $this->writerWorksheets[$i] = new Worksheet($this->strTotal, $this->strUnique, $this->strTable, $this->colors, $this->parser, $this->preCalculateFormulas, $this->phpExcel->getSheet($i));
        }
        ...
        $OLE = new File(OLE::Asc2Ucs($workbookStreamName));
        ...
        if (isset($this->documentSummaryInformation) && !empty($this->documentSummaryInformation)) {
            $OLE_DocumentSummaryInformation = new File(OLE::Asc2Ucs(chr(5) . 'DocumentSummaryInformation'));
        ...
        if (isset($this->summaryInformation) && !empty($this->summaryInformation)) {
            $OLE_SummaryInformation = new File(OLE::Asc2Ucs(chr(5) . 'SummaryInformation'));
        ...
        $root = new OLE_PPS_Root(time(), time(), $arrRootData);
        // save the OLE file
