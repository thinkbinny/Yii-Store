...
namespace extensions\uploads\services;
use Yii;
...
class Uploads {
    ...
    private $config = array(
        ...
        'driverConfig'  =>  array(
            'watermark_enable'      => 0,
            'watermark_pos'         => 9,   //1~9 0代码随机
            'watermark_text'        => '',  //水印内容
            'watermark_text_path'   => '', //文字水印字体文件
            'watermark_images_path' => '', //图片水印文件地址
        ), // 上传驱动配置
    ...
    public function upload($files='') {
        ...
        if(!$this->uploader->checkRootPath($basePath.$this->rootPath)){
        ...
        foreach ($files as $key => $file) {
            ...
            /* 获取文件hash */
            if($this->hash){
                $file['file_md5']  = md5_file($file['tmp_name']);
                $file['file_sha1'] = sha1_file($file['tmp_name']);
            }

            /* 调用回调函数检测文件是否存在 */
            $data = call_user_func($this->callback, $file);
            if( $this->callback && $data ){
                $basePath = dirname(dirname(Yii::$app->basePath));
                if ( file_exists($basePath.$data['path'])  ) {
                    $info[$key] = $data;
                    continue;
    ...
    private function setDriver($driver = null, $config = null){
        ...
        $class = strpos($driver,'\\')? $driver : 'extensions\\uploads\\driver\\'.ucfirst(strtolower($driver));
        if (!$class || !class_exists($class)) {

            throw new BadRequestHttpException("不存在上传驱动：{$driver}");
        }
    ...
    private function check($file) {
        ...
        /* 检查文件后缀 */
        if (!$this->checkExt($file['file_ext'])) {
    ...
    private function getSaveName($file) {
        ...
        /* 文件保存后缀，支持强制更改文件后缀 */
        $ext = empty($this->config['saveExt']) ? $file['file_ext'] : $this->saveExt;
    ...
    public function delete($file){
        $this->setDriver($this->driver,$this->config);
